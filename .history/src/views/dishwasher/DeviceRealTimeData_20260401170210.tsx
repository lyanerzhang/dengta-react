...
/** 接口 content 可能是数组，或包在 list / records / data 等字段中 */
function normalizeApiList<T>(raw: unknown): T[] {
  if (Array.isArray(raw)) return raw as T[]
  if (raw != null && typeof raw === "object") {
    const o = raw as Record<string, unknown>
    for (const key of ["list", "records", "data", "rows", "items"]) {
      const v = o[key]
...
export default function DeviceRealTimeData() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      setDataList(normalizeApiList(listRes))
    } finally {
      setLoading(false)
    }
  }
  ...
  const fetchFilters = async () => {
    ...
    setCityList(normalizeApiList(cities))
    setStoreList(normalizeApiList(stores))
  }
